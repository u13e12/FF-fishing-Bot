...
# Global variables
exclamation_mark = Vision('DoubleMark1.jpg')
...
stormfish_caught = Vision('Stormfish.jpg')
surfaceslap_available = Vision('SurfaceSlapAvailable.jpg')
repeat = 0
    ...
    while(True):
        ...
        if status == FishingStatus.Reeling:
            while status == FishingStatus.Reeling:
                ...
                fish_got_away = error_fish_got_away.find(
                    screenshot, 0.9, 'rectangles')
                stormfish = stormfish_caught.find(
                    screenshot, 0.9, 'rectangles')
                ...
                if fish_got_away:
                    ...
                    pyautogui.moveTo(random.randint(888, 958), random.randint(
                        624, 638), 0.3, pyautogui.easeInQuad)
                    ...
                    screenshot11 = wincap.get_screenshot()
                    surfaceslap = surfaceslap_available.find(
                        screenshot11, 0.95, 'rectangles')
                    time.sleep(random.uniform(0.25, 0.5))
                    if surfaceslap:
                        pyautogui.press((KeyBindings.get('SurfaceSlap')))
                    status = FishingStatus.Success
